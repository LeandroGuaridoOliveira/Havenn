...
async function processOrderAndGetLink(orderId: string, setDownloadInfo: any, setError: any, setLoading: any) {
    ...
    try {
        ...
        const orderDetailsRes = await fetch(`http://localhost:3000/orders/${orderId}`, {
        ...
        const webhookRes = await fetch(`http://localhost:3000/orders/${orderId}/status`, {
            method: 'PATCH',
            headers: authHeaders, 
        ...
        const downloadRes = await fetch(`http://localhost:3000/orders/${orderId}/download/${firstItem.productId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
