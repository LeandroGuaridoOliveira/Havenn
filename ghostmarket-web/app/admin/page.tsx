...
import { UploadCloud, CheckCircle, AlertCircle, PackagePlus, LogOut } from "lucide-react";

export default function AdminPage() {
  const router = useRouter();
  const [isAuthorized, setIsAuthorized] = useState(false); // Controla se mostra a tela
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push('/login'); // Chuta para o login se não tiver token
    } else {
      setIsAuthorized(true); // Libera a tela
    }
  }, [router]);
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!file) {
      setStatus({ type: 'error', msg: "É obrigatório anexar um arquivo (.zip, .rar, .exe)." });
      return;
    }

    setStatus({ type: 'loading', msg: "Enviando produto e arquivo..." });
    ...
    try {
      // Recupera o token para enviar no cabeçalho (Futuro: quando protegermos a rota POST)
      const token = document.cookie.split('; ').find(row => row.startsWith('havenn_token='))?.split('=')[1];

      const res = await fetch("http://localhost:3000/products", {
        method: "POST",
        headers: {
          'Authorization': `Bearer ${token}` // Prepara para autenticação no backend
        },
        body: data,
      });
  ...
  function handleLogout() {
    document.cookie = "havenn_token=; path=/; max-age=0"; // Deleta o cookie
    router.push('/login');
  }

  // Se ainda não verificou o token, não mostra nada (evita piscar a tela)
      ...
      <main className="max-w-3xl mx-auto px-6 pt-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-white/[0.02] border border-white/[0.05] p-8 rounded-3xl shadow-2xl">
          ...
          <div className="pt-4 border-t border-white/[0.05]">
            <label className={labelClass}>Arquivo do Produto</label>
            <div className="relative group">
              ...
              />
              <div className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center transition-all duration-300 ${file ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-zinc-800 bg-[#0c0c0e] group-hover:border-zinc-600'}`}>
                {file ? (
                  <>
                    <CheckCircle className="text-emerald-500 mb-2" size={32} />
                    <span className="text-white font-medium">{file.name}</span>
                    <span className="text-xs text-emerald-400 mt-1">Pronto para envio</span>
                  </>
                ) : (
                  <>
                    <UploadCloud className="text-zinc-500 mb-2 group-hover:text-white transition-colors" size={32} />
                    <span className="text-zinc-400 font-medium group-hover:text-white transition-colors">Clique ou arraste o arquivo aqui</span>
                    <span className="text-xs text-zinc-600 mt-1">.zip, .rar, .exe (Max 100MB)</span>
                  </>
                )}
              </div>
            </div>
